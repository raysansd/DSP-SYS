...
<br>";
 	...
 	$tunggak_spp = mysqli_query($konek, "SELECT spp.*,siswa.nis,siswa.nama_siswa,siswa.id_kelas FROM spp INNER JOIN siswa USING (nis) WHERE  siswa.id_kelas = '$_GET[kelas]' AND  spp.no_bayar is NULL ORDER BY siswa.nis ASC ");
 	$no=1;
 	$total_spp=0;
...
<br>";
 	...
 	$tunggak_dsp = mysqli_query($konek, "SELECT dsp.*,siswa.nis,siswa.nama_siswa,siswa.id_kelas FROM dsp INNER JOIN siswa USING (nis) WHERE  siswa.id_kelas = '$_GET[kelas]' AND  dsp.tanggal_bayar is NULL ORDER BY siswa.nis ASC ");
 	$no=1;
 	$total_dsp = 0;
